...
module('Unit | Service | distance', function(hooks) {
  ...
  test('the distance between two points is calculated accurately', function(assert) {
    ...
    assert.equal(distance.between(postOffice, guinnessStorehouse), '1.92', 'the distance between Dublin post office and the Guinness Storehouse is the same as google maps reports');
    assert.equal(distance.between(postOffice, drumraneyChurch), '99.74', 'the distance between Dublin post office and Drumraney Church is the same as google maps reports');
  });
});
